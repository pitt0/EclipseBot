...
class ReaperBow(Weapon):
    ...
    def __init__(self, character):
        ...
        self.BA = f"Deals {self.User.Strength * 0.95:.0f} _(95% of your `Strength`)_ + 5% of your `Strength` per target's `Death` stack and applies 1 `Death` stack. At 4 stacks the target takes 100% of their `Armor` as `Ability` damage and removes all of their `Death` stacks."
        # self.Description
        self.Abilities = {
            "Ghost Link": {
                "Brief": "You shot a Spectral Arrow that pierces the target",
                "Description": f"You shot a Spectral Arrow that pierces the target dealing {self.User.Strength * 0.8:.0f} _(80% of your `Strength`)_ + 25% of target's `Armor` and applying 1 `Death` stack\n\n_25 Stamina_",
                "Stamina": 25,
                "Callable": self.GhostLink,
                "Targets": 1
            },
            "Relentless Slinger": {
                "Brief": "You aim at a __vital point__ of the enemy and shoot a Spectral Arrow",
                "Description": f"You shoot a Spectral Arrow to a __Vital Point__ of the target dealing {self.User.Strength * 0.9:.0f} _(90% of your `Strength`)_ + 30% of target's `Armor`\n\n_42 Stamina_",
                "Stamina": 42,
                "Callable": self.RelentlessSlinger,
                "Targets": 1
            },
            "Shadow Showdown": {
                "Brief": "You free your Soul in the form of a `Shadow` Arrow, attacking up to three enemies at once",
                "Description": f"You attack up to three enemies suffering 34 `Health Points` for each enemy hit, dealing {self.User.Strength * 0.75:.0f} damage _(75% of your `Strength`)_ and lowering their `Strength` by 12 * `Death` stacks on target\n\n_31 Stamina_",
                "Stamina": 31,
                "Callable": self.ShadowShowdown,
                "Targets": 3
            },
            "Ruined Quiver": {
                "Brief": "You let your Bow seize yourself, increasing your damage",
                "Description": "You let your Bow seize yourself, increasing your `Strength` by 15 * `Death` stack on the field and taking 23 damage\n\n_31 Stamina_",
                "Stamina": 31,
                "Callable": self.RuinedQuiver,
                "Targets": 0
            }
        }
    ...
    def GhostLink(self, target: 'Player', allies: list['Player']):
        self.User.cStamina -= self.Abilities["Ghost Link"]["Stamina"]
    ...
    def RelentlessSlinger(self, target: 'Player', allies: list['Player']):
        self.User.cStamina -= self.Abilities["Relentless Slinger"]["Stamina"]
    ...
    def ShadowShowdown(self, target: list['Player'], allies: list['Player']):
        self.User.cStamina -= self.Abilities["Shadow Showdown"]["Stamina"]
    ...
    def RuinedQuiver(self, target: 'Player', allies: list['Player']):
        self.User.cStamina -= self.Abilities["Ruined Quiver"]["Stamina"]
